...
export class HttpResponseInterceptor implements HttpInterceptor {
  ...
  private errorHandler = (err: HttpErrorResponse) => {
    ...
    if(err.error instanceof Error) {
      ...
      this.msgService.add({severity: 'error', summary: 'Unknown error', detail: 'Report this to the admin or developers!'});
    } else {
      this.msgService.add({severity: 'error', summary: err.name, detail: `Backend returned code ${err.status} : ${err.error.message ? err.error.message : 'Unknown error'}`});
    }
    return EMPTY;
  }
}
